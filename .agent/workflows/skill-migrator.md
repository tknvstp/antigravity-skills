...
该文档包含：
- 核心工具清单和使用场景
- 模式系统（PLANNING / EXECUTION / VERIFICATION）
- Artifact 系统（task.md / implementation_plan.md / walkthrough.md）
- Workflow 机制和 Turbo 注解
...
迁移不仅是格式转换，更是能力增强。以下是 Antigravity 独有能力：

| Claude Code 能力 | Antigravity 增强 | 迁移优化场景 |
|-----------------|-----------------|-------------|
| 无浏览器能力 | `browser_subagent` | 添加在线研究、UI 验证步骤 |
| 无图像生成 | `generate_image` | 添加资产生成能力 |
| 手动任务追踪 | `task.md` artifact | 复杂 workflow 自动追踪 |
| 隐式模式切换 | `task_boundary` 显式管理 | 明确指导模式流转 |
| 混合用户交互 | `notify_user` 结构化通信 | 添加用户确认检查点 |
...
- 方案设计完成后（BlockedOnUser: true）
- 关键变更执行前
- 验证完成时（BlockedOnUser: false）
```
...
| 阶段 | 模式 | 主要工具 |
|-----|------|---------|
| 分析原始 Skill | PLANNING | `view_file`, `grep_search` |
| 制定迁移计划 | PLANNING | 创建 `implementation_plan.md` |
| 创建目录结构 | EXECUTION | `run_command`, `write_to_file` |
| 迁移文件 | EXECUTION | `write_to_file`, `run_command` |
| 验证迁移 | VERIFICATION | 创建 `walkthrough.md` |
...
**使用 task.md**：创建迁移任务清单追踪进度。

**提问清单**（如信息不明确，使用 `notify_user`）：
- 这个技能应该自动触发还是用户显式调用？
- 有哪些资源文件需要保留？
...
**完成后**：使用 `notify_user` 通知用户验证结果，创建 `walkthrough.md` 记录迁移摘要。
...
| 工具 | 用途 | 迁移优化场景 |
|-----|------|-------------|
| `task_boundary` | 任务模式切换 | 为复杂 workflow 添加模式切换指导 |
